...
#       author={Nikhil Raina and Guruprasad Somasundaram and Kang Zheng and
#           Sagar Miglani and Steve Saarinen and Jeff Meissner and Mark Schwesinger
#           and Luis Pesqueira and Ishita Prasad and Edward Miller and Prince Gupta
#           and Mingfei Yan and Richard Newcombe and Carl Ren and Omkar M Parkhi},
#       year={2023},
#       eprint={2308.13093},
#       archivePrefix={arXiv},
#       primaryClass={cs.CV}
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Scale detections by the given factor to allow blurring more area, 1.15"
            " would mean 15% scaling"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Absolute path where we want to store the unblurred image array",
    )
...
def create_output_directory(file_path: str) -> None:
    """
    parameter file_path: absolute path to the directory where we want to create the
        output files
    Simple logic to create output directories if they don't exist.
    """
    print(
        f"Directory {os.path.dirname(file_path)} does not exist. Attempting to create"
        " it..."
    )
    os.makedirs(os.path.dirname(file_path))
    if not os.path.exists(os.path.dirname(file_path)):
        raise ValueError(
            f"Directory {os.path.dirname(file_path)} didn't exist. Attempt to create"
            " also failed. Please provide another path."
        )
...
def validate_inputs(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if not 1 <= args.output_video_fps or not (
        ...
        raise ValueError(
            f"Invalid output_video_fps {args.output_video_fps}, should be a positive"
            " integer"
        )
    ...
    if args.output_image_path is not None and not os.access(
        ...
        raise ValueError(
            f"You don't have permissions to write to {args.output_image_path}. Please"
...
def get_detections(
    ...
    parameter model_score_threshold: model score threshold to filter out
        low confidence detection
    parameter nms_iou_threshold: NMS iou threshold to filter out low
        confidence overlapping boxes
...
def visualize(
    ...
    parameter scale_factor_detections: scale detections by
        the given factor to allow blurring more area, 1.15 would mean 15% scaling

    Visualize the input image with the detections and save the output
    ...
    for box in detections:
        ...
        unblurred_face = image_original[y1:y2, x1:x2]
        box_images.append(unblurred_face)
        box_lists.append([x1, y1, x2, y2])

    return [image, box_images, box_lists]
...
def visualize_image(
    ...
    parameter face_model_score_threshold: face model score threshold to filter
        out low confidence detection
    parameter lp_model_score_threshold: license plate model score threshold
        to filter out low confidence detection
    ...
    parameter scale_factor_detections: scale detections by the given factor to
        allow blurring more area
...
def visualize_video(
    ...
    output_image_array: str,
):
    """
    parameter input_video_path: absolute path to the input video
    parameter face_detector: face detector model to perform face detections
    parameter lp_detector: face detector model to perform face detections
    parameter face_model_score_threshold: face model score threshold to
        filter out low confidence detection
    parameter lp_model_score_threshold: license plate model score threshold to
        filter out low confidence detection
    parameter nms_iou_threshold: NMS iou threshold
    ...
    for frame in video_reader_clip.iter_frames():
        ...
        if frame_number % 100 == 0:
            print(
                f"Processing frame {frame_number} of"
                f" {video_reader_clip.duration * output_video_fps}..."
            )
        ...
        visualize_image_results = visualize(
            image,
            detections,
            scale_factor_detections,
        )
    ...
    if visualized_images:
        print(output_video_fps)
        if output_video_fps is None:
            output_video_fps = 10  # Set to default value
    ...
    if faces_list:
        ...
        np.save(output_image_array, faces_array)

    if faces_dict:
        with open("faces_dict.pickle", "wb") as handle:
            pickle.dump(faces_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
        # make csv file
