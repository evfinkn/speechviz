...
#               Sagar Miglani and Steve Saarinen and Jeff Meissner and
#               Mark Schwesinger and Luis Pesqueira and Ishita Prasad and
#               Edward Miller and Prince Gupta and Mingfei Yan and
#               Richard Newcombe and Carl Ren and Omkar M Parkhi},
#       year={2023},
#       eprint={2308.13093},
#       archivePrefix={arXiv},
#       primaryClass={cs.CV}
...
def create_output_directory(file_path: str) -> None:
    """
    parameter file_path: absolute path to the directory where we want to create
        the output files
...
def get_detections(
    ...
    parameter model_score_threshold: model score threshold to filter out low
        confidence detection
    parameter nms_iou_threshold: NMS iou threshold to filter out low confidence
        overlapping boxes
...
def visualize(
    ...
    parameter scale_factor_detections: scale detections by the given factor
        to allow blurring more area, 1.15 would mean 15% scaling

    Visualize the input image with the detections and save the output image at
        the given path
    """
...
def visualize_image(
    ...
    parameter scale_factor_detections: scale detections by the given factor to allow
        blurring more area
