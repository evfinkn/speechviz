...
def route_file(*paths: pathlib.Path, verbose=0, scan_dir=True, **kwargs):
    ...
    # run process audio on every file in file.path if it is a dir and scan_dir is True
        ...
        if path.name == "data": # the data dir was passed so run on data/audio and data/video
...
def process_audio(path: pathlib.Path, auth_token, reprocess=False, quiet=False, verbose=0, split_channels=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument("path", nargs="*", type=pathlib.Path, help="The path to the file to process. If an audio file, processes the audio file. If a directory, processes every audio file in the directory.")
