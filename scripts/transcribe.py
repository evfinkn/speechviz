...
lazies_imported = (
    False  # set to True when slower imports have been imported in transcribe()
)
...
def route_file(*args, verbose=0, scan_dir=True, **kwargs):
    ...
    if file.ext.casefold() == ".txt":
        if verbose:
            print(
                f"{file.path} is a text file. Running process_audio on the file on each"
                " line..."
            )
    ...
    # run process audio on every file in file.path if it is a dir and scan_dir is True
        ...
        ls = (
            subprocess.run(["ls", file.path], stdout=subprocess.PIPE)
            .stdout.decode()
            .split("\n")[:-1]
        )  # get files in dir
        if (
            "audio" in ls
        ):  # if "audio" dir is in file.path, run route_file on "audio" dir
            if verbose:
                print(f"Detected audio directory. Routing {file.path}/audio")
            route_file(
                f"{file.path}/audio", verbose=verbose, scan_dir=scan_dir, **kwargs
            )
        if (
            "video" in ls
        ):  # if "video" dir is in file.path, run route_file on "video" dir
            if verbose:
                if "audio" in ls:
                    print()  # visually separate audio and video sections
                print(f"Detected video directory. Routing {file.path}/video")
            route_file(
                f"{file.path}/video", verbose=verbose, scan_dir=scan_dir, **kwargs
            )
        if "audio" not in ls and "video" not in ls:
            ...
            for dir_file in ls:
                route_file(
                    f"{file.path}/{dir_file}", verbose=verbose, scan_dir=False, **kwargs
                )

    elif file.ext.casefold() in (
        ".mp3",
        ".wav",
        ".flac",
        ".ogg",
...
def get_data_dir(
    file_dir,
):  # data_dir is path to the dir containing vrs and graphical dirs
    global regex  # use global regex so it doesn't need to be re-initialized
    if "regex" not in globals():
...
def transcribe(file, reprocess=False, quiet=False, verbose=0):
    ...
    if data_dir:
        ...
        subprocess.run(
            ["mkdir", output_dir], capture_output=True
    ...
    if os.path.exists(output_path) and not reprocess:
        if not quiet or verbose:
            print(
                f"{file.path} has already been processed. To reprocess it, use the '-r'"
                " argument"
            )
        return
    ...
    data, _ = librosa.load(
        wav_file.path, sr=None
    )  # no need to set sr because already resampled
    ...
    for sample_offset in range(0, len(data), SAMPLE_INTERVAL_STEP):
        ...
        # this is where the logic starts to get the
        # start and end timestamp for each word
        ...
        ids_w_time = [
            i for i in ids_w_time if i[1] != wav2vec2_processor.tokenizer.pad_token_id
        ]
        # now split the ids into groups of ids where each group represents a word
        split_ids_w_time = [
            list(group)
            for k, group in groupby(ids_w_time, lambda x: x[1] == delim_token_id)
            if not k
        ]

        # make sure that there are the same number of
        # id-groups as words. Otherwise something is wrong
        if len(split_ids_w_time) != len(words) and (not quiet or verbose):
            print(
                f"something went wrong for sample window {sample_offset} -"
                f" {stop_sample} "
                + "(time window"
                f" {librosa.samples_to_time(sample_offset, sr=SAMPLE_RATE)}"
                + f" - {librosa.samples_to_time(stop_sample, sr=SAMPLE_RATE)})"
            )
            print(
                f"len(split_ids_w_time) == {len(split_ids_w_time)} != {len(words)} =="
                " len(words)"
            )
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="*",
        help=(
            "The path to the file to process. If an audio or "
    ...
    if not args.quiet or args.verbose:
        start_time = time.perf_counter()
    route_file(
        *args.path, reprocess=args.reprocess, quiet=args.quiet, verbose=args.verbose
    )
    if not args.quiet or args.verbose:
        time_taken = time.perf_counter() - start_time
